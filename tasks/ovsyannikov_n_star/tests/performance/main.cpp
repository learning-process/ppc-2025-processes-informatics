...
class OvsyannikovNRunPerfTestStar : public ppc::util::BaseRunPerfTests<InType, OutType> {
 protected:
  ...
  bool CheckTestOutputData(OutType& actual_res) final { return actual_res == expected_output_; }
...
TEST_P(OvsyannikovNRunPerfTestStar, RunPerfModes) { ExecuteTest(GetParam()); }

const auto kAllPerfTasks = ppc::util::MakeAllPerfTasks<InType, OvsyannikovNStarMPI, OvsyannikovNStarSEQ>(
    PPC_SETTINGS_ovsyannikov_n_star);
