...
Скалярное произведение двух векторов a = (a₁, a₂, ..., aₙ) и b = (b₁, b₂, ..., bₙ) вычисляется по формуле (в формате, совместимом с GitHub Markdown):
...
3. Для всех элементов \( i \) от 0 до \( n-1 \):  
   - Вычислить частичный результат `a[i] * b[i]`;  
