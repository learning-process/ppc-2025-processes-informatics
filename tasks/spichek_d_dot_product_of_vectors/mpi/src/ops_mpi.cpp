...
bool SpichekDDotProductOfVectorsMPI::RunImpl() {
  ...
  int local_size = (!vector1.empty() && vector1.size() == vector2.size()) ? static_cast<int>(vector1.size()) : 0;
  ...
  if (local_size > 0) {
    ...
    for (size_t i = start; i < end; ++i) {
      local_dot += static_cast<long long>(vector1[i]) * vector2[i];
    }
  }

  long long global_dot = 0;
  MPI_Allreduce(&local_dot, &global_dot, 1, MPI_LONG_LONG, MPI_SUM, MPI_COMM_WORLD);
  GetOutput() = static_cast<int>(global_dot);

  // гарантируем, что все процессы закончат вместе
