...
void ExchangeRows(std::vector<std::vector<double>>& augmented_matrix, 
                  int first_row, int second_row, int columns) {
    if (first_row == second_row) return;
    
    for (int col_idx = 0; col_idx < columns; ++col_idx) {
        std::swap(augmented_matrix[first_row][col_idx], 
                  augmented_matrix[second_row][col_idx]);
    }
}
...
void NormalizeRow(std::vector<std::vector<double>>& augmented_matrix, 
                  int row_index, double normalizer, int columns) {
    if (IsNumericallyZero(normalizer)) return;
...
int LocatePivotIndex(const std::vector<std::vector<double>>& augmented_matrix, 
                     int start_row, int column, int total_rows) {
...
void EliminateFromRow(std::vector<std::vector<double>>& augmented_matrix,
                      int target_row, int source_row, 
                      double elimination_coefficient, int columns) {
    if (IsNumericallyZero(elimination_coefficient)) return;
    
    for (int col_idx = 0; col_idx < columns; ++col_idx) {
        augmented_matrix[target_row][col_idx] -= 
            elimination_coefficient * augmented_matrix[source_row][col_idx];
    }
}

void BroadcastRow(std::vector<std::vector<double>>& augmented_matrix, 
                  int row, int columns) {
    MPI_Bcast(augmented_matrix[row].data(), columns, MPI_DOUBLE, 0, MPI_COMM_WORLD);
}

void BroadcastMatrix(std::vector<std::vector<double>>& augmented_matrix, 
...
void TransformToReducedRowEchelonFormMPI(std::vector<std::vector<double>>& augmented_matrix, 
    ...
    while (current_row < equations_count && current_col < augmented_columns - 1) {
        ...
        for (int row_idx = 0; row_idx < equations_count; ++row_idx) {
            if (row_idx == current_row) continue;
...
bool HasInconsistentEquation(const std::vector<std::vector<double>>& augmented_matrix,
                            int equations_count, int augmented_columns) {
    for (int row_idx = 0; row_idx < equations_count; ++row_idx) {
...
int ComputeMatrixRank(const std::vector<std::vector<double>>& augmented_matrix,
                     int equations_count, int augmented_columns) {
    int rank = 0;
    for (int row_idx = 0; row_idx < equations_count; ++row_idx) {
        if (!IsZeroRow(augmented_matrix[row_idx], augmented_columns - 1)) {
...
std::vector<double> ComputeSolutionVector(const std::vector<std::vector<double>>& matrix,
                                         int n, int m) {
...
void FlattenAndBroadcastMatrix(const std::vector<std::vector<double>>& matrix, 
                               int equations_count, int augmented_columns) {
...
void ReceiveAndReconstructMatrix(std::vector<std::vector<double>>& matrix, 
                                 int equations_count, int augmented_columns) {
    std::vector<double> flat_matrix(static_cast<size_t>(equations_count * augmented_columns));
    MPI_Bcast(flat_matrix.data(), equations_count * augmented_columns, MPI_DOUBLE, 0, MPI_COMM_WORLD);
...
bool GaussJordanMPI::ValidationImpl() {
    ...
    if (rank == 0) {
        ...
        bool is_valid = GetOutput().empty() && 
                       (total_elements == (columns * input_matrix.size()));
