...
class DolovVTorusTopologyFuncTests : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 ...
 protected:
  void SetUp() override {
    ...
    {
      std::string abs_path = ppc::util::GetAbsoluteTaskPath(PPC_ID_dolov_v_torus_topology, "pic.jpg");
...
TEST_P(DolovVTorusTopologyFuncTests, MatmulFromPic) {
...
const auto kPerfTestName = DolovVTorusTopologyFuncTests::PrintFuncTestName<DolovVTorusTopologyFuncTests>;

INSTANTIATE_TEST_SUITE_P(PicMatrixTests, DolovVTorusTopologyFuncTests, kGtestValues, kPerfTestName);
