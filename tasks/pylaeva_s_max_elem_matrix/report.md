...
4. Тестовые данные (`data`)
   - `matrix_3x3.txt` - матрица 3x3 содержит 1 максимальный элемент в конце
   - `matrix_5x5.txt` - матрица 5x5 содержит 1 максимальный элемент в середине
   - `matrix_100x100.txt` - матрица 100x100 содержит несколько максимальных элементов 
   - `matrix_1000x1000.txt` - матрица 1000x1000 содержит несколько максимальных элементов 
   - `matrix_2048x2048.txt` - матрица 2048x2048 содержит 1 максимальный элемент в начале
...
Ниже приведены таблицы с результатами Perf тестов.

*результаты были получены при запуске build/bin/ppc_perf_tests на 1 процессе*

| Размер данных (N*M) | SEQ версия (ms)| MPI версия (ms) | Ускорение |
|-------------------|----------------|------------------|----------|
| 1 000 000         |  0,7            |  1,7             | 0,41     |
| 25 000 000        | 13,7          | 34,3              | 0,40     |
| 49 000 000        | 26,9            | 65,1              | 0,41     |

*результаты были получены при запуске build/bin/ppc_perf_tests на 2 процессах* 

| Размер данных (N*M) | SEQ версия (ms)| MPI версия (ms) | Ускорение |
|-------------------|----------------|------------------|----------|
| 1 000 000         | 1,1             | 2              | 0,55     |
| 25 000 000        | 15,5          | 37,2              | 0,42     |
| 49 000 000        | 31,8            | 71,5              | 0,44     |

*результаты были получены при запуске build/bin/ppc_perf_tests на 4 процессах*

| Размер данных (N*M) | SEQ версия (ms)| MPI версия (ms) | Ускорение |
|-------------------|----------------|------------------|----------|
| 1 000 000         | 1,2             | 3,1             | 0,39     |
| 25 000 000        | 26,6          | 58,1              | 0,46     |
| 49 000 000        | 44,6            | 108,3              | 0,41     |
