...
- Реализация MPI позволяет избежать выделения временных буферов больше, чем нужно для частичных сумм.
...
## Данные
- Функциональные тесты используют 5 входных файлов: `matrix1.txt`, `matrix2.txt`, `matrix3.txt`, `matrix4.txt`, `matrix5.txt`
...
1. **Релизация MPI выполняется медленнее, чем SEQ**, поскольку:
   - Матрица (128 MB) передается на все процессы → большие накладные расходы при передаче.
   - Разложение по столбцам обеспечивает минимальную вычислительную нагрузку на процесс.
   - Сбор и передача результатов приводит к дополнительным затратам на синхронизацию.
2. **Задача имеет низкую вычислительную трудоёмкость**.
3. **Версия SEQ работает очень быстро** поскольку:
- Хорошая локальность кэша (хранение столбцов происходит построчно)
- Простые, предсказуемые циклы

4. **MPI pipeline и task_run показывают схожие накладные раскходы**, в основном за счёт:
   - MPI_Bcast матрицы размером 128 MB
   - MPI_Gatherv для результатов ~32 KB
...
- Версия MPI демонстрирует **очень низкое ускорение (0,08–0,15)** и низкую эффективность из-за:
  ...
  - низкой вычислительной сложности
