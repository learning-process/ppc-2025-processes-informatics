...
Последовательная версия реализована в классе `KhruevAGlobalOptSEQ`. Алгоритм работает итерационно. Вместо того чтобы просто перебирать сетку, он «исследует» функцию там, где это наиболее перспективно.
Основные этапы:
1. **Развертка (1D -> 2D):** Функция d2xy переводит отрезок $[0, 1]$ в квадрат. Это позволяет искать минимум функции одной переменной $t$, которая «пробегает» по всей площади квадрата.
2. **Начало:** Ставятся две точки в границах отрезка ($t=0$ и $t=1$), и в них вычисляется значение функции.
3. **Оценка крутизны (М):** Программа вычисляет параметр $M$ (оценку константы Липшица). Это число показывает, насколько резко функция может «падать» или «расти». Чем больше $M$, тем осторожнее ведет себя алгоритм.
4. **Вычисление характеристик (R):**
    - Для каждого интервала между уже найденными точками считается «перспективность» ($R$).Интервал получит высокий $R$, если он длинный (мы там еще не были) или если значения функции на его концах маленькие (рядом с ямой).
5. **Новая точка:** Алгоритм выбирает один интервал с самым большим $R$ и ставит в нем новую точку. Точка ставится не посередине, а с учетом разности значений функции на концах (формула 7).
6. Остановка: Если длина самого перспективного интервала стала меньше epsilon, алгоритм считает, что минимум найден.
---
