...
bool GonozovLSimpleIterationMethodMPI::RunImpl() {
  ...
  // MPI_Scatterv((proc_rank == 0) ? matrix.data() : nullptr, sendcounts_for_matrix.data(), displs_for_matrix.data(), MPI_DOUBLE,
  ...
  // {
  //   local_previous_approximations[i] = (block_of_b[i] / strings_of_matrix[i + additional_value_diag + number_unknowns * i]);
  // }
  ...
  //     * previous_approximations[(i + 2) % number_unknowns]) / strings_of_matrix[i + additional_value_diag + number_unknowns * i];
  //   }
...
// Разбрасываем данные
MPI_Scatterv((proc_rank == 0) ? matrix.data() : nullptr,
    sendcounts_for_matrix.data(), displs_for_matrix.data(),
    MPI_DOUBLE, local_matrix.data(), local_size * number_unknowns,
    MPI_DOUBLE, 0, MPI_COMM_WORLD);

MPI_Scatterv((proc_rank == 0) ? b.data() : nullptr,
    sendcounts_for_b.data(), displs_for_b.data(),
    MPI_DOUBLE, local_b.data(), local_size,
    MPI_DOUBLE, 0, MPI_COMM_WORLD);
...
MPI_Allgatherv(local_previous.data(), local_size, MPI_DOUBLE,
    previous_approximations.data(), sendcounts_for_b.data(),
...
for (int iter = 0; iter < max_number_iteration; iter++) {
    ...
    MPI_Allgatherv(local_current.data(), local_size, MPI_DOUBLE,
        current_approximations.data(), sendcounts_for_b.data(),
        displs_for_b.data(), MPI_DOUBLE, MPI_COMM_WORLD);
...
//MPI_Bcast(current_approximations.data(), current_approximations.size(), MPI_DOUBLE, 0, MPI_COMM_WORLD);
// for (int l = 0; l < number_unknowns; l++)
//   GetOutput().push_back(current_approximations[l]);
