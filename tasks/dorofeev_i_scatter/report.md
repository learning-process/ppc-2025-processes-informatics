...
Для измерения времени использовалось среднее значение между режимами `pipeline` и `task_run`.

#### Усреднённые времена выполнения:

| Mode    | Processes | Time (s)        | Speedup  | Efficiency |
| ------- | --------- | --------------- | -------- | ---------- |
| **seq** | 1         | **0.000000048** | **1.00** | —          |
| **mpi** | 2         | **0.000000049** | **0.98** | **49%**    |
| **mpi** | 4         | **0.000000041** | **1.17** | **29%**    |
...
* операция Scatter **коммуникационно-ограниченная**, а не вычислительная;
* время передачи сопоставимо с накладными расходами вызовов MPI;
* при малых сообщениях доминируют задержки и синхронизация.
