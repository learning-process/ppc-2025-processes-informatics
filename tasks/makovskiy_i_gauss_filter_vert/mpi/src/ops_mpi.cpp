      ...
      for (int x = 0; x < strip_width; ++x) {
        ...
        for (int ky = -1; ky <= 1; ++ky) {
          for (int kx = -1; kx <= 1; ++kx) {
            ...
            if (current_x < 0) {
              pixel_val = (rank > 0 && all_strip_widths[rank - 1] > 0)
                              ? left_ghost[std::clamp(current_y, 0, total_height - 1)]
                              : get_pixel(local_strip, current_x, current_y, strip_width, total_height);
            } else if (current_x >= strip_width) {
              pixel_val = (rank < world_size - 1 && all_strip_widths[rank + 1] > 0)
                              ? right_ghost[std::clamp(current_y, 0, total_height - 1)]
                              : get_pixel(local_strip, current_x, current_y, strip_width, total_height);
            } else {
