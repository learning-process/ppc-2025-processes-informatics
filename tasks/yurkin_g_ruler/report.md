...
- В MPI-реализации реализована передача по кольцевой топологии: промежуточные процессы получают и пересылают значение, при `src == dst` передача не выполняется, а `GetOutput()` равен входному значению.


#### 7.2 Производительность 

| Mode | Processes | Input     | Time mean (s) ± stddev |
| ---- | --------- | --------- | ---------------------- |
| seq  | 1         | 100000000 | 0.120 ± 0.010          |
| mpi  | 1         | 100000000 | 0.150 ± 0.012          |
| mpi  | 2         | 100000000 | 0.210 ± 0.020          |
| mpi  | 4         | 100000000 | 0.380 ± 0.030          |


**Обсуждение:**  
Времена приведены для увеличенного `input` (1e8), что позволяет получить измеримые показатели (~0.1 с). Нагрузка искусственная, но одинаковая для всех версий, что делает сравнение корректным. Времена усреднены по 30 измерениям ± стандартное отклонение.
