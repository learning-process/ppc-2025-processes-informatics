...
class EgorovaLBroadcastFuncTest : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 public:
  static std::string PrintTestParam(const TestType &test_param) { return std::get<1>(test_param); }
...
TEST_P(EgorovaLBroadcastFuncTest, RunTests) { ExecuteTest(GetParam()); }
...
INSTANTIATE_TEST_SUITE_P(Broadcast, EgorovaLBroadcastFuncTest,
