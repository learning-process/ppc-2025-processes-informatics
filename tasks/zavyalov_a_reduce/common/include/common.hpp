...
// процесса-получателя
using InType =
    std::tuple<MPI_Op, MPI_Datatype, size_t, void *, int>;  // void* instead of vector because input type can differ
using OutType = std::tuple<void *, bool>;  // result of mpi_reduce(void* instead of vector because input type can
