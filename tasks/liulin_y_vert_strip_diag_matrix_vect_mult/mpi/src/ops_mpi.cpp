...
int LiulinYVertStripDiagMatrixVectMultMPI::TournamentMax(const std::vector<int> &column) {
  if (column.empty()) {
...
std::vector<int> ComputeLocalMaxes(const std::vector<int> &local_data, int rows, int local_cols) {
  ...
  for (int col_idx = 0; col_idx < local_cols; ++col_idx) {
    ...
    local_maxes[col_idx] = LiulinYVertStripDiagMatrixVectMultMPI::TournamentMax(column);
  }
  return local_maxes;
}
}  // namespace
