...
class PikhotskiyRScatterPerfTests : public ppc::util::BaseRunPerfTests<InputType, OutputType> {
    ...
    static bool IsSequentialTest() {
        ...
        return test_name.find("sequential") != std::string::npos ||
    ...
    void SetUp() override {
        ...
        if (is_root) {
            std::size_t total_send_elements = sequential_mode ? 
                kElementsPerProcess : 
                static_cast<std::size_t>(kElementsPerProcess) * total_processes;
...
void RunPerformanceTestsImpl(
    const testing::TestParamInfo<ppc::util::PerfTaskInfo<InputType>>& /*test_info*/) {
...
const auto kAllPerfTasks = ppc::util::MakeAllPerfTasks<InputType, 
    PikhotskiyRScatterMPI, PikhotskiyRScatterSEQ>(PPC_SETTINGS_pikhotskiy_r_scatter);
...
// NOLINTNEXTLINE
INSTANTIATE_TEST_SUITE_P(RunModeTests, PikhotskiyRScatterPerfTests, 
                        kGtestParamValues, kPerfTestNaming);
