...
class PikhotskiyRScatterFunctionalTests : 
    public ppc::util::BaseRunFuncTests<InputType, OutputType> {
    ...
    void PrepareTestData() {
        ...
        if (is_root) {
            std::size_t total_elements = sequential_mode ? 
                elements_per_process : 
                static_cast<std::size_t>(elements_per_process) * total_processes;
...
const auto all_functional_tasks = ppc::util::MakeAllFuncTasks<InputType, 
    PikhotskiyRScatterMPI, PikhotskiyRScatterSEQ>();

const auto gtest_parameters = ppc::util::TupleToGTestValues(all_functional_tasks);

INSTANTIATE_TEST_SUITE_P(FunctionalityTests, PikhotskiyRScatterFunctionalTests, 
                        gtest_parameters, &PikhotskiyRScatterFunctionalTests::GetTestName);
