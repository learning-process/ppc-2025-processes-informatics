 ...
 protected:
  void SetUp() override {
    ...
    if (test_id == 1) {
      ...
      auto func = [x0, y0](double x, double y) -> double {
        return ParaboloidFunction(x, y, x0, y0);
      };
  ...
  bool CheckTestOutputData(OutType &output_data) final {
    ...
    std::cout << test_id << " x_min: " << output_data.x_min << " y_min: " << output_data.y_min << " value: " << output_data.value << std::endl;
...
const auto kTestTasksList =
    std::tuple_cat(ppc::util::AddFuncTask<LevonychevIMultistep2dOptimizationMPI, InType>(kTestParam, PPC_SETTINGS_levonychev_i_multistep_2d_optimization),
                   ppc::util::AddFuncTask<LevonychevIMultistep2dOptimizationSEQ, InType>(kTestParam, PPC_SETTINGS_levonychev_i_multistep_2d_optimization));

const auto kGtestValues = ppc::util::ExpandToValues(kTestTasksList);

const auto kPerfTestName = LevonychevIMultistep2dOptimizationFuncTests::PrintFuncTestName<LevonychevIMultistep2dOptimizationFuncTests>;

INSTANTIATE_TEST_SUITE_P(Multistep2dOptimizationTests, LevonychevIMultistep2dOptimizationFuncTests, kGtestValues, kPerfTestName);
