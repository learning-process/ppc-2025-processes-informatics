...
bool LevonychevIMultistep2dOptimizationMPI::RunImpl() {
  ...
  if (rank == 0) {
    ...
    for (int p = 0; p < size; ++p) {
      ...
      double x_max_proc = (p == size - 1) ? params.x_max 
                                           : params.x_min + (p + 1) * width_per_process;
      
      initial_regions[p] = SearchRegion(x_min_proc, x_max_proc, params.y_min, params.y_max);
    }
  }
  MPI_Scatter(initial_regions.data(), 
              sizeof(SearchRegion), MPI_BYTE,
              &my_region, sizeof(SearchRegion), MPI_BYTE,
              0, MPI_COMM_WORLD);

  for (int step = 0; step < params.num_steps; ++step) {
    int grid_size = params.grid_size_step1 * (1 << step);

    std::vector<Point> local_points = SearchInRegion(
        params.func, my_region, grid_size);

    int num_local_candidates = std::min(params.candidates_per_step,
                                       static_cast<int>(local_points.size()));
    std::vector<Point> local_candidates(
        local_points.begin(),
        local_points.begin() + num_local_candidates);
    ...
    MPI_Gather(local_candidates.data(), params.candidates_per_step * sizeof(Point), MPI_BYTE,
               ...
               params.candidates_per_step * sizeof(Point), MPI_BYTE,
               0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
      ...
      int num_global_candidates = std::min(params.candidates_per_step,
                                          static_cast<int>(valid_candidates.size()));
      all_candidates.assign(valid_candidates.begin(),
                           valid_candidates.begin() + num_global_candidates);
    }
    ...
    if (step < params.num_steps - 1) {
      if (rank == 0) {
        ...
        if (!all_candidates.empty()) {
          for (const auto& cand : all_candidates) {
            ...
            SearchRegion new_region(
                std::max(params.x_min, cand.x - margin_x),
                std::min(params.x_max, cand.x + margin_x),
                std::max(params.y_min, cand.y - margin_y),
                std::min(params.y_max, cand.y + margin_y));
            
            new_regions.push_back(new_region);
          }
        } else {
          new_regions.push_back(SearchRegion(params.x_min, params.x_max,
        ...
        if (new_regions.empty()) {
          for (int p = 0; p < size; ++p) {
            regions_to_scatter[p] = SearchRegion(params.x_min, params.x_max,
                                                 params.y_min, params.y_max);
          }
          ...
          for (int p = 0; p < size; ++p) {
            ...
            regions_to_scatter[p] = SearchRegion(x_min_proc, x_max_proc,
                                                 base_region.y_min, base_region.y_max);
          }
        }

        MPI_Scatter(regions_to_scatter.data(), sizeof(SearchRegion), MPI_BYTE,
                   &my_region, sizeof(SearchRegion), MPI_BYTE,
                   0, MPI_COMM_WORLD);
      } else {
        MPI_Scatter(nullptr, sizeof(SearchRegion), MPI_BYTE,
                   &my_region, sizeof(SearchRegion), MPI_BYTE,
                   0, MPI_COMM_WORLD);
      }
  ...
  if (my_region.x_min < my_region.x_max && my_region.y_min < my_region.y_max) {
    std::vector<Point> local_points = SearchInRegion(
        params.func, my_region, params.grid_size_step1 * (1 << (params.num_steps - 1)));
    
    if (!local_points.empty()) {
      my_best = local_points[0];
      if (params.use_local_optimization) {
        my_best = LocalOptimization(params.func, my_best.x, my_best.y,
  ...
  MPI_Gather(&my_best, sizeof(Point), MPI_BYTE,
            (rank == 0) ? all_final_points.data() : nullptr,
            sizeof(Point), MPI_BYTE,
            0, MPI_COMM_WORLD);
  ...
  if (rank == 0) {
    ...
    if (!valid_points.empty()) {
      ...
      global_best = Point((params.x_min + params.x_max) / 2.0,
                         (params.y_min + params.y_max) / 2.0,
                         params.func((params.x_min + params.x_max) / 2.0,
                                    (params.y_min + params.y_max) / 2.0));
    }
  }
