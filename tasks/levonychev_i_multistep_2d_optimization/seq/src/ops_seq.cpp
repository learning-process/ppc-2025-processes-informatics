...
bool LevonychevIMultistep2dOptimizationSEQ::RunImpl() {
  ...
  for (int step = 0; step < params.num_steps; ++step) {
    ...
    int num_candidates = std::min(params.candidates_per_step, static_cast<int>(grid_points.size()));
    candidates.assign(grid_points.begin(), grid_points.begin() + num_candidates);
