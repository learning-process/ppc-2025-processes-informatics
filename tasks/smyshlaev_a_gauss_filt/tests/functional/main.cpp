...
class SmyshlaevAGaussFiltRunFuncTestsProcesses : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 public:
  // Имя теста теперь находится под индексом 4
 ...
 protected:
  void SetUp() override {
    ...
    // Собираем InType (ImageType) из компонентов кортежа
...
// Вспомогательная функция теперь возвращает кортеж (TestType)
...
const std::array<TestType, 8> kTestParam = {
    CreateRandomTest(3, 3, 3, "3x3_RGB"),
