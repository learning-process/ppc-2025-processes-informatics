...
class NikitinAVecSignRotationFuncTests : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 ...
 protected:
  void SetUp() override {
    // Получаем параметры теста и извлекаем TestType из tuple
    auto params = GetParam();
    TestType test_params = std::get<2>(params);  // TestType находится на третьей позиции
    ...
    switch (test_case_id_) {
      ...
      case 13: // Граничные значения
        input_data_ = {std::numeric_limits<double>::min(), 
 ...
 private:
  ...
  std::vector<double> GenerateAlternatingVector(size_t size) {
