...
class OddEvenSortPerfTest : public ppc::util::BaseRunPerfTests<InType, OutType> {
 protected:
  ...
  bool CheckTestOutputData(OutType &res) override {
    if (ShouldRun()) {
      if (res.size() != input_.size()) return false;
...
const auto kPerfTasks = ppc::util::MakeAllPerfTasks<InType, OddEvenSortMPI, OddEvenSortSeq>(
    PPC_SETTINGS_volkov_a_odd_even_transposition
);
