...
TEST_P(KosolapovVCalcMultIntegralsMSimpsonPerfTestProcesses, RunPerfModes) {
  ExecuteTest(GetParam());
}

const auto kAllPerfTasks =
    ppc::util::MakeAllPerfTasks<InType, KosolapovVCalcMultIntegralsMSimpsonMPI, KosolapovVCalcMultIntegralsMSimpsonSEQ>(
        PPC_SETTINGS_kosolapov_v_calc_mult_integrals_m_simpson);

const auto kGtestValues = ppc::util::TupleToGTestValues(kAllPerfTasks);

const auto kPerfTestName = KosolapovVCalcMultIntegralsMSimpsonPerfTestProcesses::CustomPerfTestName;

INSTANTIATE_TEST_SUITE_P(RunSimpsonModeTests, KosolapovVCalcMultIntegralsMSimpsonPerfTestProcesses, kGtestValues,
                         kPerfTestName);
