...
class RozenbergARadixSimpleMergeFuncTests : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 public:
 ...
 protected:
  void SetUp() override {
    ...
    if (CheckTestAndRank()) {
      ...
      std::string abs_path = ppc::util::GetAbsoluteTaskPath(PPC_ID_rozenberg_a_radix_simple_merge, filename);
...
TEST_P(RozenbergARadixSimpleMergeFuncTests, RadixSimpleMerge) {
...
const auto kTestTasksList = std::tuple_cat(ppc::util::AddFuncTask<RozenbergARadixSimpleMergeMPI, InType>(
                                           ...
                                           ppc::util::AddFuncTask<RozenbergARadixSimpleMergeSEQ, InType>(
                                               kTestParam, PPC_SETTINGS_rozenberg_a_radix_simple_merge));
