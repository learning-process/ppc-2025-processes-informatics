...
GaseninLIntRecMethSEQ::GaseninLIntRecMethSEQ(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = 0;
}

bool GaseninLIntRecMethSEQ::ValidationImpl() {
  const auto &[str1, str2] = GetInput();
  return str1.length() <= 10000 && str2.length() <= 10000;
}

bool GaseninLIntRecMethSEQ::PreProcessingImpl() {
  return true;
}

bool GaseninLIntRecMethSEQ::RunImpl() {
  const auto &[str1, str2] = GetInput();
...
bool GaseninLIntRecMethSEQ::PostProcessingImpl() {
  return true;
}

InType GaseninLIntRecMethSEQ::ReadInteractive() {
  InType input;
...
InType GaseninLIntRecMethSEQ::ReadFromFile(const std::string &filename) {
  InType input;
...
void GaseninLIntRecMethSEQ::PrintResult(const InType &input, OutType result) {
  std::cout << "\n=== Результат сравнения ===\n";
...
int main(int argc, char *argv[]) {
  try {
    gasenin_l_int_rec_meth::InType input;
    ...
    if (argc > 1) {
      ...
      if (arg == "--file" || arg == "-f") {
        if (argc > 2) {
          input = gasenin_l_int_rec_meth::GaseninLIntRecMethSEQ::ReadFromFile(argv[2]);
        } else {
      ...
      // Интерактивный режим
      input = gasenin_l_int_rec_meth::GaseninLIntRecMethSEQ::ReadInteractive();
    }

    // Создаем и выполняем задачу
    gasenin_l_int_rec_meth::GaseninLIntRecMethSEQ task(input);
    ...
    // Выводим результат
    gasenin_l_int_rec_meth::GaseninLIntRecMethSEQ::PrintResult(input, task.GetOutput());
