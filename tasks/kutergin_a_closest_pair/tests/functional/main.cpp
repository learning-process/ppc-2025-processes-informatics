...
class KuterginAClosestPairFuncTests : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 ...
 protected:
  void SetUp() override {
    ...
    for (auto v : input_data_) std::cout << v << ' ';
...
const std::array<TestType, 10> kTestParam = {
    std::make_tuple(std::make_tuple(v1,2), "test1"),
    std::make_tuple(std::make_tuple(v2,0), "test2"),
    std::make_tuple(std::make_tuple(v3,1), "test3"),
    std::make_tuple(std::make_tuple(v4,0), "test4"),
    std::make_tuple(std::make_tuple(v5,-1), "test5"),
    std::make_tuple(std::make_tuple(v6,-1), "test6"),
    std::make_tuple(std::make_tuple(v7,0), "test7"),
    std::make_tuple(std::make_tuple(v8,1), "test8"),
    std::make_tuple(std::make_tuple(v9,0), "test9"),
    std::make_tuple(std::make_tuple(v10,2), "test10")
};

  const auto kTestTasksList = std::tuple_cat(ppc::util::AddFuncTask<KuterginAClosestPairMPI, InType>(kTestParam, PPC_SETTINGS_kutergin_a_closest_pair),
      ppc::util::AddFuncTask<KuterginAClosestPairSEQ, InType>(
          kTestParam, PPC_SETTINGS_kutergin_a_closest_pair));

  const auto kGTestValues = ppc::util::ExpandToValues(kTestTasksList);

  const auto kNameGen =
      KuterginAClosestPairFuncTests::PrintFuncTestName<KuterginAClosestPairFuncTests>;
