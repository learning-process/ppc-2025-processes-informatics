...
std::vector<int> DistributeData(int rank, int size, int n, const std::vector<int>& v) {
    ...
    if (rank == 0) {
        ...
        for (int i = 1; i < size; ++i) {
            ...
            MPI_Send(v.data() + other_start, other_end - other_start, 
                    MPI_INT, i, 0, MPI_COMM_WORLD);
        }
    } else {
        MPI_Recv(local_data.data(), static_cast<int>(local_data.size()), 
...
int CheckBoundary(int rank, int size, int end, int n, 
                 const std::vector<int>& v, const std::vector<int>& local_data, 
