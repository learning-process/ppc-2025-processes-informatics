...
class StronginFuncTests : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 public:
  static std::string PrintTestParam(const TestType &param) {
    const std::string left_tag = std::string(param.left < 0.0 ? "M" : "P") +
                   std::to_string(static_cast<int>(std::abs(param.left)));
    const std::string right_tag = std::string(param.right < 0.0 ? "M" : "P") +
                    std::to_string(static_cast<int>(std::abs(param.right)));
...
const auto kTaskList = std::tuple_cat(
    ppc::util::AddFuncTask<StronginSearchSeq, InType>(kTestCases, PPC_SETTINGS_global_search_strongin),
