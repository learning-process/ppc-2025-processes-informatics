...
bool StronginSearchSeq::PreProcessingImpl() {
  ...
  if (!PointsAreSorted(points_)) {
    std::sort(points_.begin(), points_.end(), [](const SamplePoint &lhs, const SamplePoint &rhs) {
      return lhs.x < rhs.x;
    });
  }
...
bool StronginSearchSeq::RunImpl() {
  ...
  while (iterations_done_ < max_iterations) {
    ...
    for (std::size_t i = 1; i < points_.size(); ++i) {
      ...
      if (score < best_interval_score || (std::fabs(score - best_interval_score) < 1e-12 && length > best_interval_length)) {
