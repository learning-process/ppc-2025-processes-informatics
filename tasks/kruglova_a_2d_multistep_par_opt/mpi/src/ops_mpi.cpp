...
double Solve1DStrongin(const std::function<double(double)> &func, double a, double b, double eps, int max_iters,
  ...
  for (int iter = 0; iter < max_iters; ++iter) {
    ...
    double m_val = (m_max > 0.0) ? (r_param * m_max) : 1.0;
    ...
    for (size_t i = 0; (i + 1) < trials.size(); ++i) {
      ...
      double rate = (m_val * dx) + ((dz * dz) / (m_val * dx)) - (2.0 * (trials[i + 1].z + trials[i].z));
      if (rate > max_rate) {
        max_rate = rate;
        best_idx = i;
      }
    ...
    double x_new = (0.5 * (trials[best_idx + 1].x + trials[best_idx].x)) -
                   ((trials[best_idx + 1].z - trials[best_idx].z) / (2.0 * m_val));
...
bool KruglovaA2DMuitMPI::RunImpl() {
  ...
  for (int iter = 0; iter < in.max_iters; ++iter) {
    ...
    if (rank == 0) {
      ...
      if (rates.empty() || ((trials[rates[0].idx + 1].x - trials[rates[0].idx].x) < in.eps)) {
        ...
        for (int i = 0; i < size; ++i) {
          ...
          selected_intervals[static_cast<size_t>(i)] = {trials[s_idx].x, trials[s_idx + 1].x, trials[s_idx].f,
                                                        trials[s_idx + 1].f};
        }
      }
    }
    ...
    double m_loc_max = std::abs(my_interval.f2 - my_interval.f1) / dx_local;
    double m_local = (m_loc_max > 0.0) ? (2.0 * m_loc_max) : 1.0;
    ...
    if (rank == 0) {
      for (int i = 0; i < size; ++i) {
        Trial2D res{recv_res[static_cast<size_t>(i) * 3], recv_res[static_cast<size_t>(i) * 3 + 1],
                    recv_res[static_cast<size_t>(i) * 3 + 2]};
