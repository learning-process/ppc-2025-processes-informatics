...
namespace {
    ...
    int SimpsonCoefficient(int node_index, int partitions) {
        if (node_index == 0 || node_index == partitions) return 1;
        return (node_index % 2 == 0) ? 2 : 4;
    }
}

AshihminDCalculateIntegralsBySimpsonMPI::AshihminDCalculateIntegralsBySimpsonMPI(
    const InType& input) {
...
bool AshihminDCalculateIntegralsBySimpsonMPI::ValidationImpl() {
  ...
  if (input.partitions <= 0 || input.partitions % 2 != 0) return false;
  
  for (size_t index = 0; index < input.left_bounds.size(); ++index) {
    if (input.right_bounds[index] <= input.left_bounds[index]) return false;
  }
  return true;
}
...
double AshihminDCalculateIntegralsBySimpsonMPI::IntegrandFunction(
    const std::vector<double>& coordinates) {
  return Function(coordinates);
}

void AshihminDCalculateIntegralsBySimpsonMPI::RecursiveIntegration(
    int dimension, std::vector<double>& coordinates, 
    const std::vector<double>& step_sizes, double& local_sum,
