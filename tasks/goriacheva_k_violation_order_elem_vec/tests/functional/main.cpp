...
class NesterovARunFuncTestsProcesses : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 ...
 protected:
  void SetUp() override {
    ...
    {
      std::string abs_path = ppc::util::GetAbsoluteTaskPath(PPC_ID_goriacheva_k_violation_order_elem_vec, "pic.jpg");
...
const auto kTestTasksList =
    std::tuple_cat(ppc::util::AddFuncTask<GoriachevaKViolationOrderElemVecMPI, InType>(kTestParam, PPC_SETTINGS_goriacheva_k_violation_order_elem_vec),
                   ppc::util::AddFuncTask<GoriachevaKViolationOrderElemVecSEQ, InType>(kTestParam, PPC_SETTINGS_goriacheva_k_violation_order_elem_vec));
