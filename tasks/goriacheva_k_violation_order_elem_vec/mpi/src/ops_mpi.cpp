...
void GoriachevaKViolationOrderElemVecMPI::ScatterInput(
    int rank, int size, int n, std::vector<int>& local) const {
...
int GoriachevaKViolationOrderElemVecMPI::CountLocalViolations(
...
int GoriachevaKViolationOrderElemVecMPI::CheckBoundaryViolation(
    int rank, int size, const std::vector<int>& local) const {
  if (local.empty()) {
    return 0;
  }

  const int send_val = local.back();
  int left_last = 0;

  MPI_Sendrecv(
      &send_val, 1, MPI_INT,
      (rank + 1 < size) ? rank + 1 : MPI_PROC_NULL, 1,
      &left_last, 1, MPI_INT,
      (rank > 0) ? rank - 1 : MPI_PROC_NULL, 1,
      MPI_COMM_WORLD, MPI_STATUS_IGNORE);

  if (rank > 0 && left_last > local.front()) {
    return 1;
  }
  return 0;
}
