...
Для проверки корректности реализации проведено комплексное функциональное тестирование, охватывающее различные сценарии работы алгоритма:
...
- Используется детерминистический алгоритм заполнения
...
- Ускорение (speedup) относительно последовательной версии
...
Наблюдается значительное улучшение эффективности параллелизации с ростом размера матрицы:

Для матрицы 5000×5000:
- Лучшая эффективность: 55.87% на 2 процессах
- MPI с 1 процессом медленнее sequential версии

Для матрицы 10000×10000:
- Лучшая эффективность: 61.95% на 2 процессах (+6.08%)
- MPI с 1 процессом быстрее sequential версии
