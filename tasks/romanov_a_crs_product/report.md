...
Пусть заданы матрицы $A$ и $B$ размера $n \times n$ в формате CRS. Для удобства матрица $B$ предварительно транспонируется для ускорения поиск совпадающих столбцов при перемножении строк $A$ и столбцов $B$.
...
Таким образом, фактически вычисляются только те элементы $C_{ij}$, которые потенциально могут быть ненулевыми. Это позволяет значительно сократить количество операций по сравнению с плотным алгоритмом.
