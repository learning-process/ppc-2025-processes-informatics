...
которая  интегрируема на заданном отрезке $[a,b]$. Требуется приближённо вычислить значение определённого интеграла
...
$$
\int_{a}^{b} f(x) \, dx \approx \sum_{i=1}^{n} f\left(x_{i}^{*}\right) \times \Delta x_{i}, x_{i}^{*} \in\left[x_{i-1}, x_{i}\right], \Delta x_i = x_i - x_{i-1}
$$
