...
* pipeline
* task_run
* количество MPI-процессов: 1, 2, 4, 8, 16, 32

### 6.1 Результаты измерений (task_run, MPI)

Замеры производительности проводились для MPI-реализации на 1, 2, 3, 4, 8, 16 и 32 процессах. За основу для расчетов (`task_run`) взято время выполнения на 1 процессе.

| Число процессов (P) | Время MPI (сек) | Ускорение (S) | Эффективность (E) |
|---------------------|-----------------|---------------|-------------------|
| 1                   | 0.0320          | 1.00          | 1.00              |
| 2                   | 0.0209          | 1.53          | 0.77              |
| 4                   | 0.0208          | 1.54          | 0.39              |
...
Полученные результаты демонстрируют неоднозначное поведение MPI-реализации операции Reduce при увеличении числа процессов.

При малом числе процессов (2–8) наблюдается умеренное ускорение по сравнению с последовательной версией. Максимальное ускорение достигается при использовании 8 MPI-процессов и составляет примерно 1.96. Это связано с тем, что объём вычислений на процесс остаётся достаточным для компенсации накладных расходов на обмен данными.

Однако при дальнейшем увеличении числа процессов (16 и 32) производительность резко снижается: время выполнения возрастает, ускорение становится меньше единицы, а эффективность стремится к нулю. Это объясняется следующими факторами:

* операция суммирования является вычислительно простой и имеет низкую вычислительную нагрузку;
* при росте числа процессов объём данных, обрабатываемых каждым процессом, уменьшается;
* накладные расходы на коммуникации между процессами (MPI_Scatterv, MPI_Send / MPI_Recv, MPI_Bcast) начинают доминировать над временем локальных вычислений.

Таким образом, для данной задачи существует оптимальное число процессов, после превышения которого использование большего количества MPI-процессов становится неэффективным. Наблюдаемое поведение соответствует закону Амдала, согласно которому ускорение параллельной программы ограничено долей последовательных операций и коммуникационных издержек.

---

## 8. Заключение

В ходе выполнения лабораторной работы были реализованы и исследованы последовательная и параллельная (MPI) версии операции Reduce для вычисления суммы элементов целочисленного вектора.

В рамках работы были получены следующие результаты:

* реализованы корректные версии алгоритма для SEQ и MPI, успешно прошедшие все функциональные тесты;
* MPI-реализация корректно обрабатывает входные данные произвольного размера и работает при любом числе процессов;
* экспериментальные исследования показали, что MPI-версия демонстрирует ускорение только при ограниченном числе процессов;
* при увеличении числа процессов сверх оптимального значения производительность ухудшается из-за роста коммуникационных накладных расходов.

Таким образом, применение MPI для операции Reduce целесообразно лишь при достаточно большом объёме вычислений на процесс. Для задач с простой операцией и небольшим объёмом данных последовательная реализация или использование малого числа процессов оказывается более эффективным.
