# Умножение плотных матриц (строчная декомпозиция)
...
## 1. Введение
Задача умножения плотных матриц хорошо иллюстрирует разницу между последовательной и распределённой работой с памятью. Текущая MPI-реализация под тем же именем «Кэннон» использует более простую 1D-раскладку по строкам: матрица $A$ делится между процессами, матрица $B$ целиком рассылается всем, после чего строки $C$ собираются обратно.
