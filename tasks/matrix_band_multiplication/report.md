...
**Коммуникация.** Размеры матриц передаются через `MPI_Bcast`, строки `A` распределяются `MPI_Scatterv`. Столбцы `B` сначала пакуются на корневом процессе и также рассылаются `MPI_Scatterv`, после чего полосы циклически ротируются между процессами посредством `MPI_Sendrecv`, исключая глобальное вещание всей матрицы. Блоки результата собираются на корневом процессе с помощью `MPI_Gatherv`; при необходимости матрица `C` распространяется обратно.  
**Синхронизация.** Количество коллективных операций сведено к рассылке размеров, строки `A` и сборке результата; ротация полос `B` использует попарный обмен без дополнительных барьеров.
...
|-----------:|----------:|-------------|-----------:|
| seq        | 1         | pipeline    | 0.0937927 |
| seq        | 1         | task_only   | 0.0931488 |
| mpi        | 8         | pipeline    | 0.0202471 |
| mpi        | 8         | task_only   | 0.0162914 |

Замеры получены утилитой `ppc_perf_tests.exe` (матрицы 640 × 640, один прогон). На таких данных время каждого теста превышает 0.01 секунды, поэтому разброс измерений невелик и ускорение заметно. Полная конвейерная цепочка на восьми процессах остаётся примерно в 4.6 раза быстрее последовательной реализации; накладные расходы рассылки полос `B` и сборки результата не перекрывают выигрыш от параллельных вычислений.
