...
const std::array<TestType, 16> kTestParam = {
    // 1. Базовые случаи
    std::make_tuple(std::string("Hello world"), 2), std::make_tuple(std::string("One two  three   four"), 4),
    std::make_tuple(std::string("   Leading and trailing spaces   "), 4),
    std::make_tuple(std::string("Word-with-hyphen counted-as-one"), 2), std::make_tuple(std::string(""), 0),
    std::make_tuple(std::string("a"), 1),

    // 2. Пунктуация и символы
    std::make_tuple(std::string("Hello, world!"), 2), std::make_tuple(std::string("Wait... what?! Really?!"), 3),
