...
void LeonovaAMostDiffNeighVecElemsMPI::FindLocalMaxDiff(const std::vector<int> &local_data, int &local_max_diff,
  ...
  if (local_data.size() == 1) {
    ...
    long long diff = static_cast<long long>(local_data[index]) - static_cast<long long>(local_data[index + 1]);
    long long curr_diff_ll = std::llabs(diff);//safe abs
    
    int curr_diff = (curr_diff_ll <= static_cast<long long>(std::numeric_limits<int>::max())) 
