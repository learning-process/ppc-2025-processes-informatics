...
bool SannikovIColumnSumMPI::ValidationImpl() {
  ...
  return (!input_matrix.empty()) && (!input_matrix.front().empty()) && (GetOutput().empty());
}
...
bool SannikovIColumnSumMPI::PreProcessingImpl() {
  ...
  int columns = 0;
  if (rank == 0) {
    columns = static_cast<int>(input_matrix.front().size());
  }
...
void SannikovIColumnSumMPI::PrepareSendBuffer(const InType &input_matrix, int rank, int rows, int columns,
                                              std::vector<int> &sendbuf) {
  if (rank != 0) {
    return;
  }
  if (rank == 0) {
    const int base = rows * columns;
    sendbuf.resize(base);
...
bool SannikovIColumnSumMPI::RunImpl() {
  ...
  int rows = 0;
  int columns = 0;
  columns = static_cast<int>(GetOutput().size());
  ...
  for (int i = 0; i < size; i++) {
    elem_for_proc[i] = static_cast<int>(base / size) + (i < (base % size) ? 1 : 0);
