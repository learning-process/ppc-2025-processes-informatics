...
1.  Определяется общая длина сравнения (`total_len`), равная максимальной длине из двух строк.
2.  Общая область сравнения делится на равные блоки (чанками) между всеми процессами (`size`). Каждый процесс (`rank`) получает свой локальный диапазон `[start, end)`.

### 4.2. Локальный поиск
Каждый процесс выполняет локальное сравнение строк A и B в своем диапазоне `[start, end)`. Локальное сравнение реализовано во вспомогательной функции `FindLocalDifference` для снижения когнитивной сложности.

1.  Процесс ищет первое различие в своем чанке.
2.  Если различие найдено на позиции $i$:
    * Записывается позиция $i$ (`local\_diff\_pos`).
    * Записывается локальный результат сравнения (`local\_result`: $-1$ или $1$).
    * Поиск прекращается (`break`).
3.  Если различие не найдено, позиция `local_diff_pos` устанавливается в `total_len`.

### 4.3. Коммуникация (Двухэтапный Allreduce)
Для определения истинного результата требуется два этапа глобальной коммуникации:

**Этап 1: Определение глобальной позиции первого различия.**
* Процессы обмениваются своими локальными позициями различия (`local\_diff\_pos`).
...
* Только процесс, который нашел различие на позиции `global\_min\_pos`, имеет ненулевой локальный результат (`local\_result`). Для всех остальных процессов результат сбрасывается в `0`.
* Выполняется операция **`MPI_Allreduce`** с функцией **`MPI_SUM`** над всеми локальными результатами. Поскольку только один процесс может иметь ненулевой результат (на самой первой глобальной позиции), сумма даст корректный итоговый результат сравнения ($-1$, $0$ или $1$).

### 4.4. Обработка граничных случаев
После двух `MPI_Allreduce` алгоритм проверяет:
* Если `global_min_pos == total_len`, это означает, что строки совпали до конца. В этом случае итоговый результат определяется сравнением исходных длин строк A и B.
...
*Приведенные числа являются приблизительными и зависят от платформы.*
